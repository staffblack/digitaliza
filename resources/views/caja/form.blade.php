...
<div class="col-lg-4">
    <div class="form-group {{ $errors->has('codigo_verificacion') ? 'has-error' : ''}}">
        <label for="codigo_verificacion" class="control-label">{{ 'Codigo Verificacion' }}</label>
...
<div class="col-lg-4">
    <div class="form-group {{ $errors->has('etiqueta') ? 'has-error' : ''}}">
        ...
        <input class="form-control" name="etiqueta" type="file" id="etiqueta" value="{{ isset($caja->etiqueta) ? $caja->etiqueta : ''}}" >
...
<div class="col-lg-4">
    <div class="form-group {{ $errors->has('ubicacion') ? 'has-error' : ''}}">
        ...
        <select id="ubicacion" name="ubicacion" class="form-control">
            <?php
                include 'conexion3.php';
                $registros = mysqli_query($conexion, "select * FROM ubicacion") or
    die("Problemas en el select:" . mysqli_error($conexion));

  while ($reg = mysqli_fetch_array($registros)) {
                ?>
            <option value="{{ $reg['id']}}">{{$reg['ubicacion'] }}</option> 
            <?php 
  }
            ?>
     </select>
...
<div class="col-lg-4">
    <div class="form-group {{ $errors->has('rack') ? 'has-error' : ''}}">
        ...
        <select id="rack" name="rack" class="form-control">
            <?php
                include 'conexion3.php';
                $registros = mysqli_query($conexion, "select * FROM rack") or
    die("Problemas en el select:" . mysqli_error($conexion));

  while ($reg = mysqli_fetch_array($registros)) {
                ?>
            <option value="{{ $reg['id']}}">{{$reg['rack'] }}</option> 
            <?php 
  }
            ?>
     </select>
...
<div class="col-lg-4">
    <div class="form-group {{ $errors->has('cuerpo') ? 'has-error' : ''}}">
        ...
        <select id="cuerpo" name="cuerpo" class="form-control">
            <?php
                include 'conexion3.php';
                $registros = mysqli_query($conexion, "select * FROM cuerpo") or
    die("Problemas en el select:" . mysqli_error($conexion));

  while ($reg = mysqli_fetch_array($registros)) {
                ?>
            <option value="{{ $reg['id']}}">{{$reg['cuerpo'] }}</option> 
            <?php 
  }
            ?>
     </select>
