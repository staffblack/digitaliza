        ...
        <div class="panel-heading">
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
					...
					{!! Form::select('id_curso', $cursos, old('cursos'), ['onchange' => 'cursoChangeEs()','class' => 'form-control select', 'required' => '','id' =>'id_curso']) !!}
                    ...
                    @if (count($users) > 0)
                        @foreach ($users as $user)
                            <tr data-entry-id="{{ $user->id }}">
								...
								<td>
                                <?php 
                                    $registros=mysqli_query($conexion,"select * from matriculas where id_user='$user->id'") or
      die("Problemas en el select:".mysqli_error($conexion));

    if ($reg=mysqli_fetch_array($registros))
    {
        echo $reg['ano_lectivo'];
               }                  
                                ?>
                                </td>
