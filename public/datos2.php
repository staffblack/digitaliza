...
<a href="../pdf/notasgeneral.php?iduser=<?php echo $iduser;?>"><i class="fa fa-file-pdf-o" style="font-size:30px;color:red"></i></a>
<table class="table table-bordered table-striped" border="1">		
		...
		if($_SESSION['materia']==null)
			{
			$registros=mysqli_query($conexion,"select ma.materia AS materia, no.id AS idnota ,no.id_user AS id_user, us.name AS name,us.apellido AS apellido, q1p1,q1p2,q1p3,q1exam,q2p1,q2p2,q2p3,q2exam  from notas AS no INNER JOIN users AS us ON no.id_user=us.id INNER JOIN materias AS ma ON ma.id=no.id_materia where ma.id_user='$iduser'") or
  die("Problemas en el select:".mysqli_error($conexion));

while ($reg=mysqli_fetch_array($registros))
{
?>
<form action="../admin/nota" method="get">
